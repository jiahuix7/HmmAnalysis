#include "../lib/BTagCalibrationStandalone.h"
#include <algorithm>
#include <exception>
#include <iostream>
#include <sstream>
...
BTagEntry::Parameters::Parameters(OperatingPoint op,
                                  ...
                                  std::string sys_type, JetFlavor jf,
                                  float eta_min, float eta_max, float pt_min,
                                  float pt_max, float discr_min,
                                  float discr_max)
    : operatingPoint(op), measurementType(measurement_type), sysType(sys_type),
      jetFlavor(jf), etaMin(eta_min), etaMax(eta_max), ptMin(pt_min),
      ptMax(pt_max), discrMin(discr_min), discrMax(discr_max) {
...
BTagEntry::BTagEntry(const std::string &csvLine) {
    ...
    params = BTagEntry::Parameters(BTagEntry::OperatingPoint(op), vec[1],
                                   vec[2], BTagEntry::JetFlavor(jf),
                                   stof(vec[4]), stof(vec[5]), stof(vec[6]),
                                   stof(vec[7]), stof(vec[8]), stof(vec[9]));
}
...
std::string th1ToFormulaBinTree(const TH1 *hist, int start = 0, int end = -1) {
    ...
    if (start == end - 1) { // no parenthesis for neighbors
        char tmp_buff[70];
        sprintf(tmp_buff, "x<%g ? %g:%g", hist->GetXaxis()->GetBinUpEdge(start),
                hist->GetBinContent(start), hist->GetBinContent(end));
        return std::string(tmp_buff);
    }
...
class BTagCalibrationReader::BTagCalibrationReaderImpl {
  ...
  private:
    BTagCalibrationReaderImpl(
        BTagEntry::OperatingPoint op, const std::string &sysType,
    ...
    double eval_auto_bounds(const std::string &sys, BTagEntry::JetFlavor jf,
                            float eta, float pt, float discr) const;
...
void BTagCalibrationReader::BTagCalibrationReaderImpl::load(
    const BTagCalibration &c, BTagEntry::JetFlavor jf,
    std::string measurementType) {
    ...
    for (const auto &be : entries) {
        ...
        if (op_ == BTagEntry::OP_RESHAPING) {
            te.func = TF1("", be.formula.c_str(), be.params.discrMin,
                          be.params.discrMax);
        } else {
            te.func =
...
double BTagCalibrationReader::BTagCalibrationReaderImpl::eval_auto_bounds(
    const std::string &sys, BTagEntry::JetFlavor jf, float eta, float pt,
    float discr) const {
...
BTagCalibrationReader::BTagCalibrationReaderImpl::min_max_eta(
    BTagEntry::JetFlavor jf, float discr) const {
    bool use_discr = (op_ == BTagEntry::OP_RESHAPING);
...
double BTagCalibrationReader::eval(BTagEntry::JetFlavor jf, float eta, float pt,
                                   float discr) const {
...
std::pair<float, float>
BTagCalibrationReader::min_max_pt(BTagEntry::JetFlavor jf, float eta,
                                  float discr) const {
