...
void RoccoR::init(std::string filename) {
    ...
    std::cout << "RoccoR init file: " << filename << std::endl;
    if (in.fail())
...
int RoccoR::etaBin(double x) const {
    // std::cout <<"inside of etaBin "<<NETA<<std::endl;
    for (int i = 0; i < NETA - 1; ++i) {
        // std::cout <<"i: "<<i<<" "<<etabin[i+1]<<std::endl;
        if (x < etabin[i + 1])
            return i;
    }
    // std::cout <<"inside of etaBin 2"<<std::endl;
    return NETA - 1;
}
...
double RoccoR::kScaleDT(int Q, double pt, double eta, double phi, int s,
                        int m) const {
    // std::cout <<"inside of RoccoR"<<std::endl;
    // std::cout <<Q<<" "<<pt<<" "<<eta<<" "<<phi<<" "<<s<<" "<<m<<std::endl;
    int H = etaBin(eta);
